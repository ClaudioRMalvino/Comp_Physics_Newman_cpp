...
int main() {
  ...
  while (true) {
    ...
    if (E <= V) {
      double k{-k1(E)};
      std::cout << std::fixed << std::setprecision(3) << "wavevector: " << k
      ...
      std::cout << std::fixed << std::setprecision(1)
                << "Probability of reflection: " << R(k1_val, k2_val) * 100
                << "% || wavevector: " << std::setprecision(3) << -k1_val
                << '\n';

      std::cout << std::fixed << std::setprecision(1)
                << "Probability of transmission: " << T(k1_val, k2_val) * 100
...
double k1(double E) { return std::sqrt(2 * m * E / h_bar); }

// k2 calculates reflected wave vector k2
double k2(double E, double V) { return std::sqrt(2 * m * (E - V) / h_bar); }
